...
import { Prisma, Payment, Session, Tax } from "@prisma/client";
...
export class PaymentServiceBase {
  ...
  async getSessionId(parentId: string): Promise<Session | null> {
    return this.prisma.payment
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getTaxId(parentId: string): Promise<Tax | null> {
    return this.prisma.payment
      .findUnique({
        where: { id: parentId },
      })
