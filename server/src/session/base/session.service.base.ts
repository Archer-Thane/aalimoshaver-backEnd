...
export class SessionServiceBase {
  ...
  async getPayId(parentId: string): Promise<Payment | null> {
    return this.prisma.session
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getPsychoScheduleId(parentId: string): Promise<PsychoSchedule | null> {
    return this.prisma.session
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getReportId(parentId: string): Promise<Report | null> {
    return this.prisma.session
      .findUnique({
        where: { id: parentId },
      })
